...
export class CADSolver {
  ...
  async initSolver(): Promise<void> {
    try {
      ...
      // Pour l'instant, utiliser le solveur simplifié intégré
      this.gcsWrapper = new SimplifiedSolver();
      this.initialized = true;
      console.log('CAD Solver initialized (simplified mode)');
    } catch (error) {
...
class SimplifiedSolver {
  ...
  private applyConstraint(constraint: GcsConstraint): number {
    switch (constraint.type) {
      ...
      case 'p2p_distance': {
        ...
        if (!p1.fixed) { p1.x = mx - dx; p1.y = my - dy; }
        if (!p2.fixed) { p2.x = mx + dx; p2.y = my + dy; }
  ...
  get_dof(): number {
    ...
    for (const c of this.constraints) {
      switch (c.type) {
        case 'p2p_coincident': dof -= 2; break;
        case 'horizontal': dof -= 1; break;
        case 'vertical': dof -= 1; break;
        case 'p2p_distance': dof -= 1; break;
        case 'point_fixed': dof -= 2; break;
        case 'parallel': dof -= 1; break;
        case 'perpendicular': dof -= 1; break;
        case 'l2l_angle': dof -= 1; break;
        default: break;
      }
    }
