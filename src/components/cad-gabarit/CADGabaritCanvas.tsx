...
export function CADGabaritCanvas({
  ...
  }, [sketch, viewport, selectedEntities, hoveredEntity, currentSnapPoint, tempGeometry, showGrid, showConstraints, showDimensions]);
  ...
  }, [activeTool, viewport, tempPoints, sketch, currentSnapPoint, snapEnabled, selectedEntities, findEntityAtPosition, screenToWorld, solveSketch]);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (snapEnabled) {
      const snap = snapSystemRef.current.findSnapPoint(
        screenX, screenY, sketch, viewport, []
      );
  ...
  }, [isPanning, panStart, sketch, viewport, snapEnabled, tempGeometry, tempPoints, currentSnapPoint, findEntityAtPosition, screenToWorld]);
  ...
  // Ajouter contrainte
  const addConstraint = useCallback(async (type: Constraint['type'], entities: string[], value?: number) => {
  ...
  // Ajouter cotation
  const addDimension = useCallback((type: Dimension['type'], entities: string[], value: number) => {
  ...
  // Bouton outil
  const ToolButton = ({ tool, icon: Icon, label, shortcut }: { tool: ToolType; icon: any; label: string; shortcut: string }) => (
    <TooltipProvider>
      <Tooltip>
  ...
  return (
    <div
      ref={containerRef}
      className={`flex flex-col ${isFullscreen ? 'fixed inset-0 z-50 bg-white' : 'h-[700px]'}`}
    >
      {/* Toolbar */}
      <div className="flex items-center gap-2 p-2 bg-gray-100 border-b flex-wrap">
        ...
        <div className="flex items-center gap-1">
          <Button variant="ghost" size="sm" onClick={() => setViewport(v => ({ ...v, scale: v.scale * 0.8 }))} className="h-8 w-8 p-0">
            <ZoomOut className="h-4 w-4" />
          </Button>
          <span className="text-xs font-mono w-12 text-center">{Math.round(viewport.scale * 100)}%</span>
          <Button variant="ghost" size="sm" onClick={() => setViewport(v => ({ ...v, scale: v.scale * 1.2 }))} className="h-8 w-8 p-0">
        ...
        <Badge
          variant={
            sketch.status === 'fully-constrained' ? 'default' :
            sketch.status === 'under-constrained' ? 'secondary' :
            'destructive'
          }
        >
...
// Utilitaires
function distanceToLine(p: { x: number; y: number }, p1: { x: number; y: number }, p2: { x: number; y: number }): number {
  const dx = p2.x - p1.x;
  const dy = p2.y - p1.y;
