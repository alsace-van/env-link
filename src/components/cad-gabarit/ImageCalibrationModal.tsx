...
export function ImageCalibrationModal({ isOpen, onClose, image, onSkip, onCalibrate }: ImageCalibrationModalProps) {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-hidden flex gap-4">
          ...
          <div ref={containerRef} className="flex-1 bg-gray-900 rounded-lg overflow-hidden relative">
            <canvas ref={canvasRef} width={600} height={400} onClick={handleCanvasClick} className="cursor-crosshair" />

            {/* Instructions */}
            <div className="absolute bottom-2 left-2 right-2 bg-black/70 text-white text-xs p-2 rounded">
              <p>
                <strong>Cliquez</strong> pour ajouter des points de calibration.
              </p>
              <p>
                <strong>Cliquez sur 2 points</strong> pour créer une paire et saisir la distance réelle.
              </p>
            </div>
          </div>
          ...
          <div className="w-64 flex flex-col gap-4">
            ...
            <div className="bg-gray-100 rounded-lg p-3">
              ...
              <div className="space-y-1 max-h-32 overflow-y-auto">
                {Array.from(points.values()).map((point) => (
                  ...
                  >
                    <span className="cursor-pointer flex-1" onClick={() => setSelectedPointId(point.id)}>
                      Point {point.label}
                    </span>
                    <button onClick={() => deletePoint(point.id)} className="p-1 hover:bg-red-100 rounded text-red-500">
            ...
            <div className="bg-gray-100 rounded-lg p-3 flex-1">
              ...
              <div className="space-y-2 max-h-48 overflow-y-auto">
                {pairsArray.map((pair) => {
                  ...
                  return (
                    <div key={pair.id} className="bg-white p-2 rounded text-sm">
                ...
                {pairs.size === 0 && <p className="text-gray-400 text-xs">Cliquez sur 2 points pour créer une paire</p>}
              </div>
            </div>
            ...
            {hasValidCalibration && (
              <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                <h4 className="font-medium text-sm text-green-700 mb-1">✓ Calibration valide</h4>
                <p className="text-xs text-green-600">Échelle: {scale!.toFixed(4)} mm/px</p>
              </div>
            )}
          </div>
        </div>
        ...
        <DialogFooter className="flex justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={handleCalibrate} disabled={points.size < 2}>
