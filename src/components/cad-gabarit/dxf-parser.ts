...
// VERSION: 1.0 - Support LINE, CIRCLE, ARC, LWPOLYLINE
// ============================================

import { Point, Line, Circle, Arc, Bezier, Geometry, generateId } from "./types";
    ...
    if (inEntitiesSection && code === 0) {
      ...
      if (["LINE", "CIRCLE", "ARC", "LWPOLYLINE", "POLYLINE", "SPLINE", "ELLIPSE"].includes(entityType)) {
        ...
        while (i < lines.length) {
          ...
          // Stocker les données numériques
          const numValue = parseFloat(attrValue);
          if (!isNaN(numValue)) {
            // Utiliser le code comme clé, avec gestion des multiples valeurs
            const key = `${attrCode}`;
            if (entity.data[key] !== undefined) {
              // Pour les polylignes, on peut avoir plusieurs points
              const arrayKey = `${attrCode}_array`;
              if (!entity.data[arrayKey]) {
                entity.data[arrayKey] = [entity.data[key]];
              }
              (entity.data[arrayKey] as number[]).push(numValue);
            }
            entity.data[key] = numValue;
          } else {
            entity.data[`${attrCode}`] = attrValue;
          }
  ...
  for (const entity of entities) {
    switch (entity.type) {
      case "LINE": {
        ...
        const x1 = entity.data["10"] as number;
        const y1 = entity.data["20"] as number;
        const x2 = entity.data["11"] as number;
        const y2 = entity.data["21"] as number;
      ...
      case "CIRCLE": {
        ...
        const cx = entity.data["10"] as number;
        const cy = entity.data["20"] as number;
        const radius = entity.data["40"] as number;
      ...
      case "ARC": {
        ...
        const cy = entity.data["20"] as number;
        const radius = entity.data["40"] as number;
        const startAngleDeg = entity.data["50"] as number;
        const endAngleDeg = entity.data["51"] as number;
        
        if (cx !== undefined && cy !== undefined && radius !== undefined) {
          const startAngle = (startAngleDeg || 0) * Math.PI / 180;
          const endAngle = (endAngleDeg || 360) * Math.PI / 180;
      ...
      case "LWPOLYLINE": {
        ...
        const xArray = entity.data["10_array"] as number[] | undefined;
        const yArray = entity.data["20_array"] as number[] | undefined;
        
        // Si pas de tableau, c'est qu'il n'y a qu'un seul point (ou le dernier)
        const xs: number[] = xArray ? [entity.data["10"] as number, ...xArray] : 
                             (entity.data["10"] !== undefined ? [entity.data["10"] as number] : []);
        const ys: number[] = yArray ? [entity.data["20"] as number, ...yArray] : 
                             (entity.data["20"] !== undefined ? [entity.data["20"] as number] : []);
        
        // Créer des lignes entre les points successifs
        if (xs.length >= 2 && xs.length === ys.length) {
          for (let i = 0; i < xs.length - 1; i++) {
            const p1Id = getOrCreatePoint(xs[i], ys[i]);
            const p2Id = getOrCreatePoint(xs[i + 1], ys[i + 1]);
          ...
          const flags = entity.data["70"] as number;
          if (flags && (flags & 1) && xs.length > 2) {
      ...
      // SPLINE sera converti en courbes de Bézier approximatives
      case "SPLINE": {
        // Simplification: convertir les points de contrôle en segments
        // Une vraie implémentation devrait calculer les courbes B-spline
        const xArray = entity.data["10_array"] as number[] | undefined;
        const yArray = entity.data["20_array"] as number[] | undefined;
...
/**
 * Charge un fichier DXF depuis une URL ou un File
 */
