...
import { Sketch, Point, Line, Circle as CircleType, Arc, Bezier, Spline } from "./types";
...
export default function PDFPlanEditor({ sketch, isOpen, onClose, initialOptions }: PDFPlanEditorProps) {
  ...
  const addLengthDimension = useCallback(
    (lineId: string) => {
  ...
  const addRadiusDimension = useCallback(
    (entityId: string, isDiameter: boolean = false) => {
      ...
      const existing = dimensions.find(
        (d) => d.entityId === entityId && (d.type === "radius" || d.type === "diameter"),
      );
      if (existing) {
  ...
  const addPointToPointDimension = useCallback(
  ...
  const draw = useCallback(() => {
    ...
    const pageScale = Math.min((width * 0.9) / pageWidthMm, (height * 0.9) / pageHeightMm);
    const pageScreenWidth = pageWidthMm * pageScale;
    const pageScreenHeight = pageHeightMm * pageScale;
    ...
    if (options.showFrame) {
      ...
      ctx.strokeRect(
        pageX + marginPx,
        pageY + marginPx,
        pageScreenWidth - 2 * marginPx,
        pageScreenHeight - 2 * marginPx,
      );
    }
    ...
    ctx.fillRect(
      pageX + options.margin * pageScale,
      cartoucheY,
      pageScreenWidth - 2 * options.margin * pageScale,
      cartoucheH,
    );
    ctx.strokeStyle = "#000000";
    ctx.lineWidth = 1;
    ...
    ctx.fillText(
      options.date,
      pageX + pageScreenWidth - options.margin * pageScale - 50 * pageScale,
      cartoucheY + cartoucheH * 0.75,
    );

    // ===== CALCULER LA ZONE DE DESSIN DANS LA PAGE =====
    const marginPx = options.margin * pageScale;
    const drawingAreaX = pageX + marginPx;
    const drawingAreaY = pageY + marginPx;
    const drawingAreaW = pageScreenWidth - 2 * marginPx;
    const drawingAreaH = cartoucheY - pageY - 2 * marginPx;
    ...
    sketch.geometries.forEach((geo, id) => {
      ...
      if (geo.type === "line") {
        ...
        if (p1 && p2 && cp1 && cp2) {
          const t1 = sketchToPage(p1.x, p1.y);
          const t2 = sketchToPage(p2.x, p2.y);
          const tc1 = sketchToPage(cp1.x, cp1.y);
          const tc2 = sketchToPage(cp2.x, cp2.y);
          ctx.beginPath();
          ctx.moveTo(t1.x, t1.y);
    ...
    // Points (petits cercles) - seulement si échelle pas trop petite
    if (options.scale <= 5) {
      ctx.fillStyle = "#3B82F6";
      sketch.points.forEach((point) => {
        const tp = sketchToPage(point.x, point.y);
        ctx.beginPath();
    ...
    dimensions.forEach((dim) => {
      ...
      if (dim.type === "length") {
        ...
        const tc = sketchToPage(centerX, centerY);
        const tp = sketchToPage(dim.position.x, dim.position.y);
        ...
        ctx.fillStyle = "#FFFFFF";
        ctx.fillRect(tp.x - textWidth / 2 - 4, tp.y - 8, textWidth + 8, 16);
    ...
    <div className="fixed inset-0 z-50 bg-black/80 flex">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-white border-b p-2 flex items-center gap-2">
          ...
          <Button variant={activeTool === "pan" ? "default" : "outline"} size="sm" onClick={() => setActiveTool("pan")}>
