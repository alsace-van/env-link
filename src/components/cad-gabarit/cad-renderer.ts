...
// - Affichage du nom de la mesure au-dessus des mm
// - Support des mesures avec nom optionnel
//
...
export class CADRenderer {
  ...
  private _lastLogTime: number = 0; // Pour limiter les logs de debug
  ...
  render(
    ...
    options: {
      ...
      measurements?: Array<{
        id: string;
        name?: string; // MOD v3.71: Nom de la mesure
    ...
    sketch.geometries.forEach((geo, id) => {
      ...
      if (isConstruction && !showConstruction) return;
      ...
      if (geo.type === "line") {
        ...
        if (p1 && p2) {
          ...
          if (maxX >= cullLeft && minX <= cullRight && maxY >= cullTop && minY <= cullBottom) {
            ...
            // Pour les lignes de construction, utiliser une couleur spéciale
            const geoStrokeColor = isConstruction
            ...
            if (isReference) {
              ...
              selectedLinesByWidth.get(effectiveWidth)!.push({ p1, p2, isConstruction });
            } else {
              ...
              normalLinesByWidth
                .get(batchKey)!
                .push({ p1, p2, color: geoStrokeColor, width: strokeWidth, isConstruction });
            }
          }
        }
      } else {
    ...
    if (showDimensions) {
      ...
      if (!tempGeometry && !hideTempMeasure) {
        ...
        // Un rectangle = 4 lignes formant un cycle fermé avec angles droits
        const processedLines = new Set<string>();
        const lines = Array.from(sketch.geometries.entries()).filter(([_, geo]) => geo.type === "line") as [
          string,
          Line,
        ][];

        // Fonction pour trouver les lignes connectées à un point
        const findLinesAtPoint = (pointId: string): [string, Line][] => {
          return lines.filter(([_, line]) => line.p1 === pointId || line.p2 === pointId);
        };

        // Fonction pour détecter si une ligne fait partie d'un rectangle
        const isPartOfRectangle = (
          lineId: string,
          line: Line,
        ): { isRect: boolean; position: "top" | "bottom" | "left" | "right" | null } => {
          const p1 = sketch.points.get(line.p1);
          const p2 = sketch.points.get(line.p2);
          if (!p1 || !p2) return { isRect: false, position: null };

          // Vérifier si c'est une ligne horizontale ou verticale
          const dx = Math.abs(p2.x - p1.x);
          const dy = Math.abs(p2.y - p1.y);
          const isHorizontal = dy < 1 && dx > 5;
          const isVertical = dx < 1 && dy > 5;

          if (!isHorizontal && !isVertical) return { isRect: false, position: null };

          // Chercher les lignes connectées aux deux extrémités
          const linesAtP1 = findLinesAtPoint(line.p1).filter(([id]) => id !== lineId);
          const linesAtP2 = findLinesAtPoint(line.p2).filter(([id]) => id !== lineId);

          if (linesAtP1.length !== 1 || linesAtP2.length !== 1) return { isRect: false, position: null };

          // Vérifier que les lignes connectées sont perpendiculaires
          const [, connectedLine1] = linesAtP1[0];
          const [, connectedLine2] = linesAtP2[0];

          const cp1a = sketch.points.get(connectedLine1.p1);
          const cp1b = sketch.points.get(connectedLine1.p2);
          const cp2a = sketch.points.get(connectedLine2.p1);
          const cp2b = sketch.points.get(connectedLine2.p2);

          if (!cp1a || !cp1b || !cp2a || !cp2b) return { isRect: false, position: null };

          const c1dx = Math.abs(cp1b.x - cp1a.x);
          const c1dy = Math.abs(cp1b.y - cp1a.y);
          const c1Horizontal = c1dy < 1 && c1dx > 5;
          const c1Vertical = c1dx < 1 && c1dy > 5;

          const c2dx = Math.abs(cp2b.x - cp2a.x);
          const c2dy = Math.abs(cp2b.y - cp2a.y);
          const c2Horizontal = c2dy < 1 && c2dx > 5;
          const c2Vertical = c2dx < 1 && c2dy > 5;

          // Vérifier perpendiculaire
          const isPerpendicular1 = (isHorizontal && c1Vertical) || (isVertical && c1Horizontal);
          const isPerpendicular2 = (isHorizontal && c2Vertical) || (isVertical && c2Horizontal);

          if (!isPerpendicular1 || !isPerpendicular2) return { isRect: false, position: null };

          // C'est un rectangle ! Déterminer la position
          const midY = (p1.y + p2.y) / 2;
          const midX = (p1.x + p2.x) / 2;

          if (isHorizontal) {
            // Trouver le Y min/max du rectangle pour savoir si c'est top ou bottom
            const allY = [cp1a.y, cp1b.y, cp2a.y, cp2b.y];
            const minY = Math.min(...allY);
            const maxY = Math.max(...allY);
            const rectMidY = (minY + maxY) / 2;
            return { isRect: true, position: midY < rectMidY ? "top" : "bottom" };
          } else {
            // Trouver le X min/max du rectangle pour savoir si c'est left ou right
            const allX = [cp1a.x, cp1b.x, cp2a.x, cp2b.x];
            const minX = Math.min(...allX);
            const maxX = Math.max(...allX);
            const rectMidX = (minX + maxX) / 2;
            return { isRect: true, position: midX < rectMidX ? "left" : "right" };
          }
        };
        ...
        sketch.geometries.forEach((geo, id) => {
          if (geo.type === "line") {
            ...
            if (p1 && p2) {
              ...
              // Vérifier si c'est un rectangle et quelle position
              const rectInfo = isPartOfRectangle(id, line);
              if (rectInfo.isRect) {
                // Pour un rectangle, n'afficher que top et left (pas bottom et right)
                if (rectInfo.position === "bottom" || rectInfo.position === "right") {
                  return; // Skip cette ligne, on affiche déjà l'autre côté
                }
              }
              ...
              let offsetX = len > 0 ? (-dy / len) * offsetDist : 0;
              let offsetY = len > 0 ? (dx / len) * offsetDist : offsetDist;

              // Pour les rectangles, positionner les cotations à l'extérieur
              if (rectInfo.isRect && rectInfo.position === "top") {
                // Cotation au-dessus
                offsetX = 0;
                offsetY = -offsetDist;
              } else if (rectInfo.isRect && rectInfo.position === "left") {
                // Cotation à gauche
                offsetX = -offsetDist;
                offsetY = 0;
              }
    ...
    if (isRevealMode && revealBranch) {
      ...
      // Redessiner la grille dans la zone droite
      if (showGrid) {
        this.drawGrid(sketch.scaleFactor);
      }
    ...
    if (measurements.length > 0) {
      measurements.forEach((m) => {
        this.drawMeasure(m.start, m.end, measureScale, m.mm, m.angle, m.name);
      });
    }
  ...
  private drawBackgroundImages(
    ...
    for (const bgImage of sortedImages) {
      ...
      // Appliquer la rotation (en degrés, autour du centre de l'image)
      if (bgImage.rotation) {
  ...
  private drawGeometry(
    ...
    // Appliquer le style pointillé pour les lignes de construction
    if (isConstruction) {
  ...
  private drawMeasure(
    ...
    // MOD v3.71: Texte avec nom, mm et angle
    ...
    if (textName) {
      lines.push({ text: textName, color: "#004400" }); // Vert foncé pour le nom
    }
    lines.push({ text: textMm, color: "#006600" }); // Vert pour les mm
    if (textAngle) {
      lines.push({ text: textAngle, color: "#FF6600" }); // Orange pour l'angle
    }

    this.ctx.font = `bold ${fontSize}px Arial`;
    const textWidth = Math.max(...lines.map((l) => this.ctx.measureText(l.text).width));
    const padding = 3 / this.viewport.scale;
    ...
    // MOD UX: Fond semi-transparent très léger
    ...
    const startY = textY - (lineHeight * (numLines - 1)) / 2;
    lines.forEach((line, i) => {
      this.ctx.fillStyle = line.color;
      this.ctx.fillText(line.text, textX, startY + i * lineHeight);
    });
  }
