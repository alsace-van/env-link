...
// VERSION: 2.1 - Ajout mode inline pour intégration dans un onglet
// ============================================
...
interface BackupSettingsDialogProps {
  ...
  inline?: boolean; // Si true, affiche le contenu directement sans dialog
}

export const BackupSettingsDialog = ({ userId, inline = false }: BackupSettingsDialogProps) => {
  const {
    settings,
    updateSettings,
    exportLocalBackup,
    ...
    <div className="space-y-6">
      ...
      {settings.lastBackup && (
        <div className="text-sm text-muted-foreground">
          Dernière sauvegarde : {formatLastBackup(settings.lastBackup)}
        </div>
      )}
        ...
        <div className="flex items-start justify-between space-x-4">
          <div className="flex items-start space-x-3 flex-1">
            ...
            <div className="space-y-1">
              ...
              <p className="text-sm text-muted-foreground">Stockage dans le navigateur. Uniquement sur cet appareil.</p>
            </div>
          </div>
          <Switch
            id="local-backup"
          ...
          <div className="flex gap-2 pl-8">
            ...
            <Button variant="outline" size="sm" asChild>
              <label htmlFor="import-backup" className="cursor-pointer">
                ...
                Importer
                <input id="import-backup" type="file" accept=".json" className="hidden" onChange={handleFileImport} />
              </label>
            </Button>
          </div>
        )}
  ...
  // Mode inline : affiche directement le contenu
  if (inline) {
    return <BackupContent />;
  }

  // Mode dialog : affiche un bouton qui ouvre une modal
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Configurez plusieurs destinations de sauvegarde pour vos données.</DialogDescription>
        </DialogHeader>
        <BackupContent />
      </DialogContent>
    </Dialog>
