...
export function useEvolizQuotes(): UseEvolizQuotesReturn {
  ...
  // Créer un devis
  const createQuote = useCallback(async (data: EvolizQuoteInput): Promise<EvolizQuote | null> => {
    if (!evolizApi.hasConfig()) {
      toast({
        title: 'Erreur',
  ...
  const sendQuote = useCallback(async (quoteId: number, email?: string): Promise<boolean> => {
    if (!evolizApi.hasConfig()) {
      toast({
        title: 'Erreur',
        description: 'Evoliz non configuré',
  ...
  const linkQuoteToProject = useCallback(async (
    quoteId: number, 
    projectId: string
  ): Promise<boolean> => {
  ...
  }, [user?.id, getCachedQuotes, toast]);

  // Helper pour trouver un devis
  const getQuoteById = useCallback((quoteId: number): EvolizQuote | undefined => {
